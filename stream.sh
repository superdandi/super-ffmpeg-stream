...
OVERLAY_FILTER="[1]scale='max(min(iw*0.15,120),80)':-2[over];[0][over]overlay=W-w-10:H-h-10"
...
transmitir() {
    ...
    if [[ $has_overlay -eq 1 ]]; then
        cmd+=(-i "$OVERLAY")
        filter_string="[1]scale='max(min(iw*0.15,120),80)':-2[over];[0][over]overlay=W-w-10:H-h-10"
    fi
